...
def ImageResiduals(P, points2D, points3D):
    ...
    res_algebraic = res.copy()

    for res_idx in range(num_residuals):
        p3D = points3D[res_idx]
        p2D = points2D[res_idx]

        err, err_algebraic = ReprojectionError(P, p3D, p2D)
        res[res_idx * 2:res_idx * 2 + 2] = err
        res_algebraic[res_idx * 2:res_idx * 2 + 2] = err_algebraic
    return res
...
def OptimizeProjectionMatrix(P, points2D, points3D):
    ...
    result = spo.minimize(f, np.reshape(P / P[2, 3], 12), options={'disp': True}, constraints=scale_constraint,
                          tol=1e-12)
